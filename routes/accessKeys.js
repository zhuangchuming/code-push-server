...
router.get('/', middleware.checkToken, function(req, res, next) {
  ...
  accountManager.getAllAccessKeyByUid(uid).then(function(accessKeys){
...
router.post('/', middleware.checkToken, function(req, res, next) {
  ...
  accountManager.createAccessKey(uid, newAccessKey, createdBy, description).then(function(newToken){
    var info = {
...
router.delete('/:accessKey', middleware.checkToken, function(req, res, next){
  ...
  var uid = req.users.id;
  models.UserTokens.destroy({where: {tokens:accessKey, uid: uid}}).then(function(rowNum){
    res.send("");
  }).catch(function (e) {
    res.status(406).send(e.message);
  });
